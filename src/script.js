...
const roof = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({
        // map: textureLoader.load('textures/roof.jpg')
        color: '#b35f45'
    })
)
...
const door = new THREE.Mesh(
    new THREE.PlaneGeometry(2.8, 2.8, 100, 100),
    new THREE.MeshStandardMaterial({
        transparent: true,
...
const doorLamp = new THREE.Mesh(
    new THREE.CapsuleGeometry(0.15, 0.2, 2, 4),
    new THREE.MeshStandardMaterial({ color: '#ff7d46' })
)
doorLamp.position.z = 2.4 + 0.01
doorLamp.position.y = 2.5
house.add(doorLamp)

// Door lamp handle
const doorLampHandle = new THREE.Mesh(
    new THREE.CylinderGeometry(0.02, 0.02, 0.3, 8),
    new THREE.MeshStandardMaterial({ color: '#000000' })
)
doorLampHandle.rotation.z = - Math.PI * 0.5
doorLampHandle.rotation.y = - Math.PI * 0.5
doorLampHandle.position.z = 2.15 + 0.01
doorLampHandle.position.y = 2.6
doorLampHandle.position.x = 0
house.add(doorLampHandle)
...
doorLight2.position.set(0, 2.5, 1.19)
house.add(doorLight, doorLight2)
...
gui.add(doorLight2, 'intensity').min(0).max(2).step(0.001)
gui.add(doorLight2.position, 'x').min(- 5).max(5).step(0.001)
gui.add(doorLight2.position, 'y').min(- 5).max(5).step(0.001)
gui.add(doorLight2.position, 'z').min(- 5).max(5).step(0.001)
...
const tick = () =>
{
    ...
    ghost2.position.y = Math.sin(elapsedTime * 3) + Math.sin(elapsedTime * 0.5)
